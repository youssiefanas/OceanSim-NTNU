# Ported from isaacsim.replicator.mobility_gen
import enum
import os
import typing as tp
import cv2
...
from isaacsim.oceansim.utils.assets_utils import get_map_image_filename, get_map_yaml_filename
...
"""
    ...
    def ros_yaml(self, negate: bool = False) -> str:
        ...
        return self.ROS_YAML_TEMPLATE.format(
            image_filename=get_map_image_filename(),
    ...
    def save_ros(self, path: str):
        ...
        self.ros_image().save(os.path.join(path, get_map_image_filename()))
        with open(os.path.join(path, get_map_yaml_filename()), "w") as f:
    ...
    def buffered_meters(self, buffer_distance_meters: float) -> "OccupancyMap":
        ...
        Returns:
            OccupancyMap: The buffered (aka: dilated / padded) occupancy map.
        """
        # Fix division by resolution safely
        if self.resolution <= 0:
             buffer_distance_pixels = 0
        else:
    ...
    def pixel_to_world_numpy(self, points: np.ndarray):
        ...
        if self.height_pixels() > 0:
             ...
             v = np.zeros_like(points[:, 1]) # Effectively 0 if we consider 1-1=0? No, v=1 -> y_world = height. Assume 1.0?
    ...
    def check_world_point_in_freespace(self, point: Point2d) -> bool:
        ...
        # Check bounds again just in case cast led to out of bounds
        if y_px >= freespace.shape[0] or x_px >= freespace.shape[1]: 
             return False
